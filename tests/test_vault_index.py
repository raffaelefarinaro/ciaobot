...
# ---- VOCABULARY.md rendering -----------------------------------------------


def test_write_vocabulary_file_without_registry_is_byte_identical(tmp_path: Path):
    """`write_vocabulary_file` grew a `registry=` keyword for the Categories
    section (#624). Every existing caller — the CLI's `--write`, the memory pass,
    the proposal queue — passes no registry, and their generated files are read
    by the memory agent and committed to git, so a defaulted argument must render
    byte for byte what it rendered before rather than merely something valid.

    The whole file is pinned, on purpose: a header, the canonical census, the
    drift list and two tag tiers, so no branch of the renderer can move without
    this failing. A note of each kind is in the fixture for that reason.
    """
    _write(
        tmp_path / "People" / "Mo.md",
        "---\nname: Mo\ntype: person\ntags: [scanning, barcode]\n---\n# Mo\n",
    )
    _write(
        tmp_path / "Ideas" / "Thing.md",
        "---\nname: Thing\ntype: idea\ntags: [scanning]\n---\n# Thing\n",
    )
    _write(
        tmp_path / "Notes.md",
    ...
    vi.write_vocabulary_file(entries, tmp_path / "VOCABULARY.md")

    assert (tmp_path / "VOCABULARY.md").read_text(encoding="utf-8") == (
        ...
        "\n"
        "- `scanning-log` → `no canonical equivalent — decide`\n"
        "  - memory-vault/Notes.md\n"
        "\n"
        "## Tags (emerging)\n"
        "\n"
        "Two to 4 uses. Reuse when it fits; these are becoming conventions.\n"
        "\n"
        "- `scanning` (2) — personal\n"
        "\n"
        "## Tags (candidates)\n"
