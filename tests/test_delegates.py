...
def test_delegate_ceiling_is_not_settable_through_the_mcp_surface() -> None:
    ...
    workspace_update used to be an auto-approved control-plane tool, so it had
    to refuse delegate_max_mode explicitly or a model could raise its own
    ceiling. It has since left the MCP surface entirely — operator settings
    live in the PWA — which retires the hole by construction. Asserted here so
    a reintroduction cannot silently restore a model-writable ceiling.
    """
    from ciao import workspaces
    from ciao.execution_modes import AUTO_APPROVED_MCP_TOOLS

    assert "workspace_update" not in AUTO_APPROVED_MCP_TOOLS
    assert not hasattr(CiaoControlPlane, "workspace_update")
