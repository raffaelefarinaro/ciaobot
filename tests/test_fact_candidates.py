...
def test_structured_rows_parse_into_candidates() -> None:
    candidates, errors = fc.candidates_from_structured(json.dumps([
        {
            "schema": fc.SCHEMA_VERSION,
            "text": "Deploys run on Thursdays.",
            "destination": "memory",
            "source_message_ids": [1],
            "evidence_excerpt": "I always deploy on Thursdays",
            "attended": True,
        }
    ]))
    assert errors == []
    ...
    assert candidates[0].attended is True
    assert candidates[0].provenance == fc.PROVENANCE_CITED


def test_structured_rows_survive_a_json_fence() -> None:
    candidates, errors = fc.candidates_from_structured(
        '```json\n[{"text": "Works from Lisbon.", "destination": "memory"}]\n```'
    )
    assert errors == []
    assert [c.text for c in candidates] == ["Works from Lisbon."]


def test_an_unreadable_structured_row_becomes_a_review_candidate() -> None:
    """Invalid structured output is reviewable, never silently dropped.

    Each bad row still produces a candidate — addressed to ``review``, with
    the parse error attached — so the fact text a model did emit reaches a
    human instead of disappearing with the schema violation.
    """
    candidates, errors = fc.candidates_from_structured(json.dumps([
        {"text": "Works from Lisbon.", "destination": "memory"},
        {"text": "Prefers tabs.", "destination": "memory",
         "source_message_ids": ["three"]},
        {"destination": "memory", "source_message_ids": [1]},
        {"text": "Cites turn zero.", "destination": "memory",
         "source_message_ids": [0]},
    ]))
    assert len(candidates) == 4
    assert len(errors) == 3
    bad = candidates[1]
    assert bad.destination == "review"
    assert bad.text == "Prefers tabs."
    assert "non-integer" in bad.parse_error
    # Nothing recoverable in row 3, so the placeholder still reaches review.
    assert candidates[2].destination == "review"
    assert "no fact text" in candidates[2].parse_error
    # `[idx=0]` is forbidden by the prompt, so a 0 is a malformed row.
    assert "out-of-range" in candidates[3].parse_error

    verdict = fc.validate_candidate(bad, None)
    assert not verdict.ok
    assert verdict.code == fc.UNPARSABLE


def test_unreadable_structured_output_is_an_explicit_failure() -> None:
    """A payload that is not a candidate list fails loudly with no candidates."""
    for raw in ("", "not json", json.dumps({"ok": True})):
        candidates, errors = fc.candidates_from_structured(raw)
        assert candidates == []
        assert errors and errors[0]


def test_an_unsupported_schema_version_is_refused() -> None:
    with pytest.raises(fc.CandidateSchemaError):
        fc.candidate_from_dict({
            "schema": "fact-candidate/v9",
            "text": "Works from Lisbon.",
            "destination": "memory",
        })
...
def test_candidates_render_back_to_session_insights_markdown() -> None:
    """The structure is the source of truth; the Markdown is a view of it."""
    body = (
        "## User corrections\n"
        "- Durable rule: Deploys run on Thursdays. [idx=1] [memory]\n"
        "- Durable rule: Prefers spaces over tabs. [idx=4] [profile]\n"
        "## New entities\n"
        "- Person: Mo Salah - runs the Tuesday standup. [idx=7] [people: Mo Salah]\n"
    )
    rendered = fc.render_insights_markdown(
        fc.candidates_from_markdown(body),
        sections=("User corrections", "New entities"),
    )
    assert rendered == (
        "## User corrections\n"
        "- Durable rule: Deploys run on Thursdays. [idx=1] [memory]\n"
    ...
    # And the render survives a round trip back through the parser.
    assert [c.to_dict() for c in fc.candidates_from_markdown(rendered)] == [
        c.to_dict() for c in fc.candidates_from_markdown(body)
    ]


def test_an_uncited_candidate_renders_without_a_citation() -> None:
    """The round trip has to be able to represent "this archive never said"."""
    candidate = fc.FactCandidate(
        text="Deploys run on Thursdays.",
        destination="memory",
        section="User corrections",
    )
    assert candidate.as_bullet() == "- Deploys run on Thursdays. [memory]"
