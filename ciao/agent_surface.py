...
The dispatcher verifies the token with the :class:`AgentSessionRegistry`, then
runs the *same* operation the MCP adapter used to serve — resolved from the
shared module-level operation table in ``ciao.mcp_server`` and validated
through the same ``Tool.from_function`` metadata — inside the same auth
context, so every control-plane guard (workspace confinement, plan-mode gate,
unattended checks), the response envelope, argument validation and telemetry are
shared. The observable ``surface`` is ``"cli"`` in ``mcp_tool_calls.jsonl``.
...
class AgentDispatcher:
    ...
    def operations(self) -> tuple[str, ...]:
        """Every operation the dispatcher can run."""
    ...
    async def dispatch(self, token: str, op: str, arguments: dict[str, Any]) -> tuple[int, dict[str, Any]]:
        ...
        from ciao.control_plane import AgentPrincipal
        ...
        # Same scope the MCP mount used to require (``required_scopes=["ciaobot"]``);
        # every token the registry mints carries it today, so this only matters if
        # a second token type ever appears, and then it must not become a bypass.
        ...
        # validation (pydantic) is byte-for-byte the schema the operation
        # defines. This is schema construction from the operation table,
        # independent of the caller's arguments; a failure here is a server
        # fault, not a client 400, so it stays outside the argument-validation
        # try below.
        try:
        ...
        try:
            ...
            principal = AgentPrincipal.from_claims(access.claims or {})
