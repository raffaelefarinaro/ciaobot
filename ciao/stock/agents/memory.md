...
- **Cross-project preferences / environment / lessons** → the `ciao:memory` region, promoted by the user. Only facts that are true regardless of which project is open. An unattended curation run never writes the regions — it queues these in `Workspace/Memory-Proposals.md` instead; file one you discovered yourself with `ciao memory-proposal-add --kind memory --source <chat> "<fact>"` (re-filing dedupes), and promote directly only when the user asked for it.
