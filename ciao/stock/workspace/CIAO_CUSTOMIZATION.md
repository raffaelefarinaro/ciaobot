...
Canonical assets live in `skills/`, `commands/`, and `subagents/`. Project MCPs live in `.mcp.json`. `ciao sync-skills` mirrors canonical assets into the runtime-discovered `.claude/` and `.agents/` catalogs and generated opencode assets. Ciaobot commands remain available through the same `/command` syntax in chats.
