...
- **Agent handoffs (cross-provider sub-chats)**: when you explicitly ask the current agent to hand off to another route — e.g. "ask Codex to review this before answering" — it opens a sub-chat (`Claude ↔ Codex`) attached to that turn, exchanges several messages with the other provider, can relay a clarifying question back to you, then synthesizes the result. "Read-only" here means *you* cannot type into the participant, since the owner agent is the sole conduit; the participant itself has normal tool access. It only happens on an explicit request, never silently. Details: the `handoff_*` MCP tools. In practice delegates cover the work case and `adversarial_review` covers the second-opinion case.
