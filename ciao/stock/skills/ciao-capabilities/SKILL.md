...
- Automations how-to: the `schedule` and `schedule_action` MCP tool docstrings. Spawning chats: the `chat_create` MCP tool. Reaching another model: the `delegate_spawn` MCP tool and the `/critique` command. Vault read conventions are inline system-prompt policy.
