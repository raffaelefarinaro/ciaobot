...
description: Authoritative catalog of what Ciaobot can do, for capability questions and feature tours. Use whenever the user asks what Ciaobot is, what it can do, what features are available, whether it can do something specific, or how one of its features works (memory, vault, archiving, schedules, loops, routines, workspaces, projects, forks, subagents, skills, voice, models, providers, opencode, plan mode, notifications, desktop app, updates, menu bar, files, chat comments, pinned files, document previews, CSV tables and cell comments, HTML artifacts, backlinks, memory map, vault graph, note graph) — and when onboarding or giving a tour or walkthrough to a new user. Trigger on phrasings like "what can you do", "what can ciaobot do", "help me get started", "give me a tour", "can you remind me / remember / schedule", "can you fork this chat", "can you ask another provider", even when the word "Ciaobot" is not mentioned.
---
...
- Schedules and loops how-to: the `schedule`/`loop`/`schedule_action`/`loop_action` MCP tool docstrings. Spawning chats: the `chat_create` MCP tool. Reaching another model: the `chat_handover` MCP tool and the `/critique` command. Vault read conventions are inline system-prompt policy.
