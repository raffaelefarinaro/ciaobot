...
- **Vault Review**: inspect deterministic candidates for stale, orphaned, duplicate, or weakly linked notes; review evidence in the PWA or through the attended MCP operation; keep (which stamps the note's `updated:` date to today), improve links, trash, restore, or permanently delete with append-only audit records and reversible trash until confirmed deletion. A candidate left undecided simply stays in the queue.
