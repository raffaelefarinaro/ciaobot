...
3. **Memory and vault** — promote recurring high-confidence entries from `Workspace/Learnings.md` Active into CLAUDE.md (move to Promoted/Resolved; prune low-confidence entries older than 30 days); clear promoted/rejected memory proposals; fix `MEMORY.md` drift; run `ciao vault-lint` and apply low-risk fixes (dead wikilinks, orphans, near-duplicates); flag stale entity pages (frontmatter `updated` >90 days) and attribute contradictions.
