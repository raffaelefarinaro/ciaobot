...
- **`file_surface` returning `ok` does not mean the panel opened.** The tool only validates that the path exists inside the workspace; the pin itself happens client-side and is skipped, silently, on a narrow window (≤768px, so phones), when the user already has a file pinned in this chat, or when they dismissed an auto-pin here earlier. So never tell the user something is "in the panel" as if you had confirmed it. Say you surfaced it, and if they say they see nothing, that is expected behavior under those gates, not an MCP failure.
