...
- Never invoke a provider binary (`codex`, `opencode`) directly to fake any of this.
...
- Cross-provider work: `delegate_spawn` for work on another model, `adversarial_review` for an inline second opinion. **Never** search for or invoke a provider binary (like `codex` or `opencode`) directly.
