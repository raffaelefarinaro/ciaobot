...
class SyncSkillsResult:
    ...
    codex_mcps_installed: int = 0
    codex_mcps_pruned: int = 0
...
CODEX_MCP_EXCLUDES = frozenset({"ciaobot", "ciaobot-fastmcp"})
...
OPENCODE_MCP_EXCLUDES = CODEX_MCP_EXCLUDES
...
def _without_managed_codex_mcp_config(text: str) -> tuple[str, bool, set[str]]:
    """Remove Ciaobot's project-MCP block and return its prior server names."""
    ...
    names = {first or second for first, second in names}
...
def _codex_mcp_block(
    workspace: Path,
    *,
    excluded_names: set[str] | frozenset[str] = frozenset(),
) -> tuple[str, list[str]]:
    """Render project `.mcp.json` servers in Codex's TOML format.

    `.mcp.json` is the shared project source of truth. Credentials remain
    environment references; literal values are deliberately not copied into
    ...
    try:
        ...
        print(f"WARN: invalid MCP config at {source}; Codex MCP sync skipped", file=sys.stderr)
        return "", []
    ...
    lines = [CODEX_MCP_CONFIG_BEGIN]
    names: list[str] = []
    for raw_name in sorted(servers, key=str):
        name = str(raw_name).strip()
        meta = servers[raw_name]
        if (
            not name
            or name in CODEX_MCP_EXCLUDES
        ...
        if url:
            lines.extend([
                f"[mcp_servers.{json.dumps(name, ensure_ascii=False)}]",
        ...
        lines.extend([
            ...
            f"args = {json.dumps([str(arg) for arg in args], ensure_ascii=False)}",
        ])
        env = meta.get("env")
        env_lines: list[tuple[str, str]] = []
        if isinstance(env, dict):
            for key, value in sorted(env.items(), key=lambda item: str(item[0])):
                ref = _env_placeholder(value)
                if ref:
                    env_lines.append((str(key), ref))
        if env_lines:
            lines.append("")
            lines.append(f"[mcp_servers.{json.dumps(name, ensure_ascii=False)}.env]")
            lines.extend(
                f"{json.dumps(key, ensure_ascii=False)} = {json.dumps(value)}"
    ...
    if not names:
        return "", []
    lines.append(CODEX_MCP_CONFIG_END)
    return "\n".join(lines) + "\n", names


def _install_codex_mcps(workspace: Path) -> tuple[int, int]:
    """Project `.mcp.json` into Codex config without overwriting user tables."""
    ...
    try:
        ...
        return 0, 0
    unmanaged, partial_marker, previous_names = _without_managed_codex_mcp_config(original)
    if partial_marker:
        print(
            f"WARN: incomplete Ciaobot MCP marker in {config_path}; project MCPs not changed",
            file=sys.stderr,
        )
        return 0, 0
    try:
        ...
        print(
            f"WARN: invalid user-owned Codex config {config_path}: {exc}; project MCPs not changed",
            file=sys.stderr,
        )
        return 0, 0
    user_servers = parsed.get("mcp_servers") if isinstance(parsed, dict) else None
    ...
    if source.is_file():
        ...
        if not isinstance(source_data, dict):
            return 0, 0
    # A user-owned Codex table wins over the projection from `.mcp.json`.
    # Re-render with those names excluded so the generated block never creates
    # duplicate TOML tables.
    block, names = _codex_mcp_block(workspace, excluded_names=set(user_servers))
    installed = len(names)
    pruned = len(previous_names - set(names))
    if block:
        # Keep the two Ciaobot-managed blocks in a stable order. This matters
        # because both sync passes preserve user-owned TOML and are run on
        # every startup; appending blindly would swap their order each time.
        agents_marker = unmanaged.find(CODEX_CONFIG_BEGIN)
        if agents_marker >= 0:
            before = unmanaged[:agents_marker].rstrip()
            after = unmanaged[agents_marker:].lstrip()
            updated = (
                (before + "\n\n" if before else "")
                + block.rstrip()
                + "\n\n"
        ...
        updated = unmanaged if unmanaged.strip() else ""
    if updated != original:
...
def _install_opencode_agents(workspace: Path) -> tuple[int, int]:
    ...
    for source in _codex_agent_sources(workspace):
        name = source.stem
        if not _CODEX_AGENT_NAME_RE.fullmatch(name):
            continue
        try:
...
def _install_opencode_mcps(workspace: Path) -> tuple[int, int]:
    ...
    JSON has no comment syntax, so the marker-fence trick used for
    `.codex/config.toml` does not transfer. Ciaobot instead records the server
...
def sync_workspace_skills(
    ...
    *,
    ...
    codex_mcps_installed, codex_mcps_pruned = _install_codex_mcps(root)
    ...
    print(
        f"Codex: {codex_mcps_installed} project MCP server(s) synced; "
        f"{codex_mcps_pruned} stale pruned."
    )
    print(
    ...
    return SyncSkillsResult(
        ...
        codex_mcps_installed=codex_mcps_installed,
        codex_mcps_pruned=codex_mcps_pruned,
