...
# The one direction the dependency runs: the registry never imports this module
...
def entity_types_section(registry: EntityTypeRegistry) -> str:
    ...
    the `type:` value first (that is what a note carries), then the label, the
    folder it infers and what the category is for. A disabled entry is left out
    rather than struck through: it is not a `type:` any note should take.

    Deterministic, like the rest of this file: entries in registry order (stock
    order, then the user's own additions) and no timestamp, so a rebuild that
    changed nothing dirties nothing in git.
    """
    lines = [
        "## Categories\n",
        "The categories this vault is configured with, from `entity-types.yaml` "
        "in the vault root. A note's `type:` is one of the ids below.\n",
    ]
...
def write_vocabulary_file(
    entries: list[Entry], dest: Path, *, registry: EntityTypeRegistry | None = None
) -> None:
    """Write `VOCABULARY.md`.
