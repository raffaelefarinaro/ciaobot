...
- `POST /api/setup/finish` is only accepted in bootstrap mode from localhost with a matching browser origin/referer (off-localhost requests get a 403 pointing at `http://localhost:<port>`). Body: `vault_root` (required — the second-brain folder, absolute or `~` path anywhere on disk), `workspace` (optional app data folder, default `~/.ciaobot`), plus optional `vault_mode`, `push_contact`, `port`, `python`, `auth_required`, `launch_agents_dir`, `app_dir`, and `restart`. It writes the real workspace config, git-inits the vault, creates local launch artifacts, and asks the supervisor to restart into the configured workspace.
