...
- `ChatInfo.pending_question` (string, in `to_dict()` so it rides every chat list / chat object): raw AskUserQuestion JSON (`{"questions": [...]}`) set when the model paused the chat on a question. Native V2 forms persist this field until the server acknowledges `question_response` (including an explicit cancel); legacy headless question cards may still be dismissed by the next ordinary message. The PWA reads it on chat open to rebuild the interactive question picker after a reload. Empty string when no question is pending.
