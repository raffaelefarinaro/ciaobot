...
- Two per-workspace system routines ship enabled (Workspace care and Skill reflection); every run is visible under **Settings → Automation**. Install-wide failures remain visible through startup triage, housekeeping, and Automation without a weekly model turn.
...
|---|---|---|
| Workspace care | Daily | Processes uncertain memory items and learnings every night. When its persisted full-pass marker is overdue, it also rechecks aging notes, rebuilds the vault index, runs the workspace audit, and repairs only safe structural drift. |
| Skill reflection | Weekly (Sun) | Reviews repeated failures or corrections involving this workspace's user-owned skills and upserts one plain-language proposal per skill; never applies it automatically. |

Proposal-resolution helper chats carry structured ownership metadata and archive themselves only after a clean turn and after their proposal has actually left the queue. Discussion chats remain visible for the user's decision. Install health is monitored on every install: startup triage and Automation surface runtime failures, housekeeping surfaces upgrade work, and development mode adds verbose diagnostics.
