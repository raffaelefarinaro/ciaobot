...
Then open `http://localhost:8443`: with no configured workspace the server starts in bootstrap mode and the setup wizard asks one folder question — your second-brain folder, either a fresh vault scaffold (suggested `~/ciaobot-brain`) or an existing notes folder Ciaobot adapts — plus provider choice, before anything is created. App data (config, `.env`, runtime state, chat metadata) is plumbing and lives in `~/.ciaobot` by default; the Advanced section lets you change it. When you finish, it writes the config, scaffolds the vault where you chose, and (on macOS) renders the LaunchAgents and `Ciaobot.app`. Git follows the vault: setup initializes the second-brain folder as a git repository (`git init` plus a minimal `.gitignore` for OS/editor litter) since that is the user data worth versioning, and also keeps the app data folder a repo with a `.gitignore` that keeps `.env` and runtime state out of commits.
