...
**Agent-Agnostic & Standalone Vault**: The vault structure, project folders, markdown notes, and configuration files (such as `CLAUDE.md` and `MEMORY.md`) created in your workspace are standard, open files. They are designed to work seamlessly with any other AI agent, IDE assistant, or bare terminal command-line tool even if Ciaobot is not running. Ciaobot is simply an opinionated, local web interface and helper wrapper built to significantly improve the UX of interacting with your personal assistant.
...
ciao/                          Python backend (Starlette).
  ...
  skills_inventory.py          Build the Settings skill inventory from skills/ and skills-lock.json, with installed Claude badges.
...
skills/                        Canonical source for custom skills; `ciao sync-skills` syncs to .claude/skills/.
subagents/                     Canonical source for project agents (memory, secretary, researcher, doc-updater, comment-analyzer, pr-test-analyzer, silent-failure-hunter); synced to .claude/agents/.
commands/                      Canonical source for slash commands (/remember, /critique, /interrogation); synced to .claude/commands/.
...
- **Credentials Persistence:** Public installs keep GWS tokens in the profile config directories under `secrets/`. If you need to migrate those credentials between machines, use your own secret manager or a local helper outside the public repo.
...
Custom skills live in `skills/` and are symlinked into `.claude/skills/` by `ciao sync-skills` on Ciao startup. Project agents live in `subagents/` and are symlinked into `.claude/agents/`; slash commands live in `commands/` and are symlinked into `.claude/commands/`. Edit the canonical source folders (`skills/`, `subagents/`, `commands/`); they re-sync on restart. **Do not edit the generated `.claude/` dirs directly** and **do not run `npx skills update` or `gws generate-skills` ad-hoc.** See `skills/README.md` for the full flow.
