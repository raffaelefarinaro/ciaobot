...
When you archive a conversation, Ciaobot extracts decisions, useful learnings, and other durable facts. Confident facts are filed automatically; uncertain ones become proposals for you to review. Turn **Automatic session insights** off in Settings → Automations to stop model processing for new and archived chats; the explicit run action remains available for a one-time request. This lets your second brain grow from the work you actually do without making the vault a proprietary database.
...
- **Memory and archiving:** Every archived chat goes through memory extraction. Confident, state-shaped facts are filed into the right destination, while uncertain ones wait for your review; once a day, Ciaobot consolidates what was learned. The second-brain UI shows your notes and their connections so you can explore, adjust, and discuss them with the model.
