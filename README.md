...
A Claude Code project package: open this repo in Claude Code to use the assistant directly. Capabilities include web research, vault memory, Google Tasks, Google Workspace (calendar, email, docs, drive, sheets, etc.), on-demand memory curation, scheduled automations, voice transcription, and a PWA frontend.
...
ciao/                          Python backend (Starlette).
  ...
  dag.py                       Tiny DAG runner (Node kinds: bash / prompt / gate / subagent / retention; edges: ok / fail / always). Each node is timed via `job_runs.track_sync`, so Automation page shows per-node status. Used by `sched-skillevo` (`skill_evolution.py`), `sched-depcheck` (`dependency_review.py`), and `sched-523fad70` (`adoption_report.py`).
  ...
  adoption_report.py           Monthly product adoption report as a `ciao/dag.py` pipeline (generate gate → verify_reports gate → strip_images gate → cross_product bash → enrich subagent (opus) → autocheck gate → create_docs bash). The `autocheck` gate is an automated quality check that replaced the `adoption-report` skill's manual Phase-2h approval: it validates the 4 enrichment JSONs against the report text (missing/invalid JSON, empty summary, invalid color, empty note, customer name not in report, empty chart comment) and halts before create_docs if any fail. Triggers: `python3 -m ciao.adoption_report --month YYYY-MM` (full run), `--dry-run` (validate structure), `--autocheck` (run only the gate, exit 1 on failure). Ends at paired doc links; the human still does the Phase-4 browser copy-paste.
...
scripts/
  ...
  run-ciao.sh                  Start the Ciao server in dev.
...
**Setup readiness.** When `PWA_AUTH_TOKEN` is absent, Ciao starts in bootstrap mode using `~/.ciao/bootstrap` (or `CIAO_BOOTSTRAP_WORKSPACE`) and persists a generated temporary auth token under that workspace's `.runtime/`. `GET /api/setup-status` is public like startup status, because the first-run wizard needs it before a normal session exists. It reports workspace/vault/token/push-contact checks plus Claude, Pi, and Ollama readiness probes, without returning secret values. The local first-launch URL `/?setup=<token>` redeems `.runtime/setup-token` on localhost only, sets the signed PWA session cookie, deletes the one-time token, and redirects to `/`. The wizard finishes with `POST /api/setup/finish`, which is bootstrap-only and localhost-only: it writes the real workspace `.env`, scaffolds the configured vault root, creates the LaunchAgent plist and local `Ciao.app` shortcut, then requests the normal restart exit so launchd can relaunch into the real workspace.
...
- **Top-level reference docs** live directly under `memory-vault/personal/projects/` (e.g. `Ciao-Decisions.md`, `Ciao-Improvements.md`) and never "complete"; don't put real projects there.
...
- **Never restart the ciao service yourself** from inside the PWA. Apply code changes and ask Raffa to hit Deploy.
...
- `INTEGRATIONS.md`: env vars, OAuth setup, MCP connectors, Ciao server runtime knobs (CIAO_* vars), deploy.
