...
- feat(memory): detect bounded-memory rot in the daily curation routine (`942a6fc`)
- feat(pwa): Cmd/Ctrl+Shift+= and Cmd/Ctrl+Shift+- adjust --font-scale (`63464a2`)
- feat(gws): show the Google scopes actually granted, not a fixed list (`793c5fd`)
- feat(pwa): Cmd/Option+S shows and hides the sidebar (`86a51fc`)
- feat: improve chat composer and activity UI (`052eb76`)
- feat: expand composer asset pickers (`57eea4e`)
- feat(pwa): Cmd+M / Option+M opens the model picker (`d3d10be`)
- feat: include provider skills in slash picker (`cb95cc4`)
- feat(apple): generate through the logged-in user session (`2c49248`)
- feat(apple): route insights, titles and re-entry summaries on-device (`682ee25`)
- feat(pwa): land on home, and orient when reopening a chat (`3e17cc5`)
- feat(pwa): workspace number keys, and move the model picker off Cmd+M (`2cabf66`)
- feat(pwa): allow inline skill selection (`1416a08`)
- feat(pwa): add workspace lanes to home (`7ba915e`)
- feat(pwa): finish ChatSignals adoption and unify relative time (`bf09118`)
- feat(pwa): context bar and ranked action dock on the chat page (`c08c162`)
- feat(pwa): replace emoji icons with the app SVG set (`d81802a`)
- feat: add tray host disconnect recovery (`9e50b64`)
- feat(pwa): render self-contained HTML artifacts in the pinned panel (`d7786a1`)
- feat(pwa): add project automation tabs (`bac2cad`)
- feat: sync notification clears across devices (`fac4639`)
- feat(pwa): centre the brand in the pane header, and tag the view (`3177f65`)
- feat(pwa): name the page next to its nav icon, not in the header (`720f50c`)
- feat: add declarative live agent evals (`b8ab2f7`)
- feat: collapse sidebar subchats (`7e1df20`)
- feat: add public release evidence pipeline (`6399af3`)
- feat: ship bundled app via one-line installer (`975a41f`)
- feat(pwa): pick the project for a new chat from the home lane (`8bbb960`)
...
- changes (`91eda03`)
- release: prepare v0.7.2 (`1c49b82`)
- release: curate the v0.7.2 changelog and rebuild the PWA (`3d97805`)
- style(pwa): add expanding ring + glow to activity spinner (`99ba995`)
- Merge pull request #253 from raffaelefarinaro/feat/memory-state-event-audit (`3cfdf3a`)
- Merge pull request #254 from raffaelefarinaro/feat/memory-state-event-audit (`aa060b3`)
- release: renumber to 0.8.0 (`257cf69`)
- Merge pull request #255 from raffaelefarinaro/fix/deploy-error-reporting (`0124e03`)
- Merge pull request #258 from raffaelefarinaro/worktree-home-lanes-plan (`113ede9`)
- Merge branch 'develop' into fix/pinned-file-edit-mode (`8b5a093`)
- style(desktop): apply rustfmt to the host-disconnect dialog (`dca5fa6`)
- Merge pull request #260 from raffaelefarinaro/fix/pinned-file-edit-mode (`f3a6abf`)
- Merge pull request #261 from raffaelefarinaro/fix/image-capability-preflight (`96d8013`)
- Merge pull request #262 from raffaelefarinaro/feat/html-artifacts (`1125cfe`)
- wip: archive delegate subchats with their supervisor (`6adab03`)
- style(desktop): apply rustfmt to the notification imports (`8537da4`)
- Merge pull request #267 from raffaelefarinaro/fix/notifications-rustfmt (`64019f2`)
- Merge pull request #263 from raffaelefarinaro/wip/archive-delegate-subchats (`8622a4f`)
- Merge branch 'develop' into fix/project-tabs-a11y (`e2a8ef1`)
- Merge pull request #266 from raffaelefarinaro/fix/project-tabs-a11y (`8b3e134`)
- Merge remote-tracking branch 'origin/fix/home-lane-polish' into home-lane-polish-header (`015982c`)
- Merge pull request #270 from raffaelefarinaro/fix/home-lane-polish (`05bab23`)
- refactor: extract shared live eval runner (`7c115ca`)
- Merge pull request #264 from raffaelefarinaro/fix/257-title-engine-detail (`24c22b7`)
- Merge pull request #265 from raffaelefarinaro/fix/248-insights-timeout (`2fed2b5`)
- Merge pull request #269 from raffaelefarinaro/fix/256-native-sidecar-wheel (`31ce1a7`)
- Merge pull request #273 from raffaelefarinaro/fix/home-lane-polish (`0634694`)
- style: format bundled runtime lookup (`debf6a0`)
- Merge pull request #274 from raffaelefarinaro/feature/eve-architecture-enhancements (`a4a5d62`)
- Merge origin/develop into codex/one-line-installer (`c2a675e`)
- style: format the engine launch action argument slice (`1a7a8be`)
- Merge pull request #268 from raffaelefarinaro/fix/259-delegate-model-validation (`038d7b3`)
- Merge origin/develop into codex/one-line-installer (`95174e7`)
- codex/one-line-installer session commit 2026-08-12 15:04:18Z (`175e2f8`)
- Merge pull request #272 from raffaelefarinaro/codex/one-line-installer (`82877d9`)
...
- fix(web): let staged comment chips share a row (`b30158c`)
- fix(insights): backfill scanned a double-prefixed vault path (`7d9be55`)
- fix(claude): treat MessageParseError as an operational error (`50be2c3`)
- fix(insights): give the project-doc update 300s instead of 120s (`c76b06e`)
- fix: suppress chat scroll anchoring while pinned (`9d9cba2`)
- fix: apply code-review and cleanup findings (`3ac3c19`)
- fix(deploy): show why a Restart step failed, and show it once (`1638ede`)
- fix: harden comment selection anchors (`06ea00d`)
- fix: retry Codex capacity errors hourly (`f135965`)
- fix(web): show close chat on mobile and drop brain icon from working trace (`7db76f5`)
- fix: apply code-review and cleanup findings (`804b000`)
- fix: address the deferred review findings (`c6a557a`)
- fix(pwa): expire re-entry summary on new messages (`7ecbe59`)
- fix: keep delegate activity internal (`7002c88`)
- fix(web): force PinnedFilePanel remount when pinned file changes (`56dbe7b`)
- fix(pwa): resolve code-review findings in the lanes and dock work (`069f531`)
- fix(types): clear the three mypy errors blocking CI (`1b0db47`)
- fix(pwa): stop viewport settling timers leaking past teardown (`b5d77cf`)
- fix(apple): use the FoundationModels spelling both SDKs accept (`1757180`)
- fix(pwa): catch non-vision image turns with a pre-flight question (`f55f853`)
- fix(pwa): tighten image capability checks (`2ac892f`)
- fix: keep archive cascade going when a subchat fails (`20da8cc`)
- fix(title): propagate title-engine failure detail to job_runs (#257) (`0ee1456`)
- fix(insights): route classifier through the insights timeout (`4cbd1c2`)
- fix(pwa): complete the project tab ARIA pattern and token usage (`696736f`)
- fix(mcp): validate delegate_spawn model against configured models (`7a3961b`)
- fix(packaging): auto-discover ciao subpackages so new modules ship in the wheel (`81689d7`)
- fix: address the tractable code-review findings on the archive cascade (`f4abc38`)
- fix(archive): stop mid-turn delegates before archiving, and report each one (`5f32ba5`)
- fix(pwa): trust the server on which subchats archived, and warn about running ones (`d3bdbba`)
- fix(title): keep generic message for detail-less fallback (`46612ce`)
- fix(delegates): validate effective model, before cleanup, and only model errors (`ee95b23`)
- fix(delegates): validate Ollama catalog membership and OpenRouter static models (`3615365`)
- fix(delegates): require custom-provider model membership in validator (`3f4fb15`)
- fix(pwa): quieten the home lanes after live review (`f6cd263`)
- fix(pwa): one working indicator, squared tags, and a visible active lane (`69c7592`)
- fix(delegates): gate Codex exemption on custom providers and require Ollama backend (`d6a413f`)
- fix(pwa): calm the home lanes, and make Esc the universal way back (`a76b73e`)
- fix(pwa): one type scale in the chat breadcrumb, and drop the number (`33e9ff8`)
- fix(delegates): exempt only native Codex ids from model validation (`6c72044`)
- fix(pwa): stack the home lanes on their own width, not the window's (`dd7969c`)
- fix(delegates): tighten validator against unregistered custom ids, Ollama cloud keys, OpenRouter keys (`90f629d`)
- fix(pwa): an unread chat is not quiet, and stop the workspace pill wrapping (`8692052`)
- fix(pwa): lane highlight as a surface, and space the project state marks (`eeebb26`)
- fix(pwa): room for the nav label, and air after the activity pulse (`8317891`)
- fix: align tray unread state with PWA (`05c3ca4`)
- fix(delegates): filter backend-gated entries out of the claude_models fallback (`6252273`)
- fix(pwa): address the PR 270 review findings (`518f1d5`)
- fix(pwa): surface unread attention and tighten home spacing (`e64f990`)
- fix: normalize re-entry summaries (`2163453`)
- fix: allow embedded media in HTML artifacts (`db348d4`)
- fix: align unread chat status dot (`06dd5c0`)
- fix: avoid mypy variable redefinition (`b9807f3`)
- fix: make packaging checks CI-compatible (`a5dfb41`)
- fix: tighten backend model validation (`d66e11b`)
- fix: validate resolved routing bucket models (`2ede7ee`)
- fix: validate routing before chat cleanup (`fbb9c3e`)
- fix: resolve inherited OpenRouter aliases (`207bde8`)
- fix: skip release smoke on pull requests (`93304b2`)
- fix: close remaining pwa review gaps (`c7ceb3a`)
- fix: address installer review findings (`a80ff58`)
- fix: harden bundled release workflows (`112c8b8`)
- fix: validate bucket-resolved chat models (`fd76f3d`)
- fix: address installer review and CI failures (`fa634fd`)
- fix: authenticate runtime metadata lookup (`2ae031d`)
- fix: keep bundled runtime x86 buildable (`73571a8`)
- fix: address PR 274 review comments (`14a3748`)
- fix(handover): validate and canonicalize the chat model on handover (`c4675ca`)
- fix: generate valid Python launch agent arguments (`9ec8051`)
- fix: resolve mypy errors in release evidence comparison (`c1b99e2`)
- fix(handover): satisfy mypy on the custom-bucket resolver (`f418ae8`)
- fix: address second Codex review on PR 274 (`4ef705e`)
- fix(tts): restore the ciao desktop install hint in the read-aloud error (`559493e`)
- fix: address installer review feedback and pin release inputs (`5e89f33`)
- fix: compaction-Aware Session Message Loader (`835c6f0`)
- fix(onboarding): keep the wizard backend for the welcome chat (`cc83ed7`)
- fix(installer): keep the tray agent loaded and the app signature valid (`877d42e`)
- fix(pwa): pad the nav highlight and align the home status row (`df37736`)
- fix: address the v0.8.0 release review (`66ab16e`)
- fix: address the delta review and cleanup pass (`4b2143c`)
- fix: stop the empty-chat sweeps deleting an unsent draft (`bbea14f`)
- fix(pwa): stop clipping the nav label, and move the automations create action (`26c9b4c`)
- fix: make the unsent-draft exemption per client and expiring (`521a20c`)
- fix: close the review findings on the draft claims and the nav label (`b4ff89f`)
- fix: derive the draft claim from stored content, not from this tab (`b9a6c44`)
- fix: salvage an orphaned draft instead of defending the chat (`0dec161`)
- fix(pwa): guard staged attachments on close, drop the draft recovery (`21446e4`)

### Maintenance
- docs(license): add copyright line and Apache appendix (`aa4c919`)
- chore(deps): sync uv.lock to 0.7.1 (`e823ec9`)
- chore(deps): sync uv.lock to 0.7.2 (`17ee295`)
- docs(release): correct the claim that CI gates the desktop build (`be316d3`)
- chore(deps): sync notebooklm-py 0.8.0, claude-agent-sdk 0.2.134; drop stale openai tracking (`8501eb1`)
- test(web): cover retrying a turn from the error bubble (`9376ef7`)
- build(pwa): rebuild static bundle for the branch's frontend changes (`a405e9d`)
- build(pwa): rebuild static bundle (`faf3c8f`)
- docs: cover on-device insights and the workspace number keys (`3821c7a`)
- build(pwa): rebuild static bundle (`70112bc`)
- docs(plans): home screen workspace lanes and shared chat signals (`d2bd2f8`)
- docs(plans): correct ChatSignals scope and relative-time step (`dbe9331`)
- docs: add PWA design system contract (`929a97c`)
- build(pwa): declare the Node floor and fail loudly below it (`5e8fa86`)
- docs: plan for surfacing memory extraction and proposals (`641da70`)
- ci: select the newest Xcode before building the Swift sidecar (`67b50ed`)
- ci: re-run against updated develop (`12ab320`)
- ci: re-run against updated develop (`9d6c0df`)
- test: configure Ollama cloud fixture (`ef49918`)
- test: verify release wheel module contents (`5f33183`)
- chore: keep release evidence operator-driven (`bda421f`)
- test: align vault evidence paths with public API (`bfba70f`)
- docs: add v0.8.0 release evidence (`b43627d`)
- chore(deps): add wheel to the test extra (`cd3a56f`)
- docs(release): record the Node floor and failed-run debris traps (`b7042ba`)
