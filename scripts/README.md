...
| `gws-profile.sh` | Per-account wrapper for the `gws` CLI: routes profile `<name>` to `secrets/gws-<name>/`. The pre-registry names keep their legacy dirs: `personal` → `secrets/gws-personal/`, `work` → `secrets/gws/`. |
