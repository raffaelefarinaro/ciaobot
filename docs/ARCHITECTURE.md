...
ciao/                          Python backend (Starlette).
  ...
  app_settings.py              Runtime-mutable app settings (title, insights, and critique models, per-provider tier pins, the Apple Intelligence beta toggle, transcription locale, local TTS voice), persisted at `.runtime/app_settings.json` and overlaid on CiaoConfig. Edited via Settings → Models.
...
The set of runtime providers lives in one place: `ciao/provider_registry.py`. Each `ProviderDescriptor` carries the provider's id, its label forms, and dotted import paths for its factory, login command, system-skill listing, readiness probe, and CLI upgrade, plus its thinking levels, default model/bucket, and tier-pin settings attribute. Paths resolve lazily, so enumerating providers never imports an agent SDK. Everything that needs to list, label, validate, or probe providers reads the registry rather than repeating a provider-name literal, and `ciao/provider_service.py` resolves the factory and owns the live instance. Adding a provider is a descriptor plus an adapter.
