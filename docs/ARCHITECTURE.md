...
ciao/                          Python backend (Starlette).
  ...
  critique.py                  Multi-model adversarial review. Runs each reviewer through ciao/providers/oneshot.py, dispatching by the entry's `codex:` / `opencode:` prefix or to Claude Code when unprefixed. CLI: `python -m ciao.critique`. Used by the adversarial-review skill.
