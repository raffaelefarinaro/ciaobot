...
ciao/                          Python backend (Starlette).
  ...
  native_sidecar.py            Shared bridge to the `ciaobot-native` sidecar bundled in Ciaobot.app: locating the binary, the periodically refreshed capability probe, and running one subcommand. Also owns `respond()`, one-shot generation with Apple's on-device Foundation Model (chat titles, Session insights, and re-entry summaries) — it calls FoundationModels directly rather than Apple's `fm` CLI, which only ships with macOS 27.
  app_settings.py              Runtime-mutable app settings (title, insights, and critique models, per-provider tier pins, transcription locale, local TTS voice), persisted at `.runtime/app_settings.json` and overlaid on CiaoConfig. Edited via Settings → Models.
...
When a chat is archived, the raw session JSONL is filtered and passed through a fast model to extract durable insights (errors, dead ends, new entities, decisions, reusable code) which are appended as a `## Session insights` section; this section is the preferred input for memory curation and downstream automation (`ciao/insights.py`, gated by `CIAO_INSIGHTS_DISABLED`, `CIAO_INSIGHTS_MIN_TURNS`, `CIAO_INSIGHTS_MODEL`). Apple Intelligence is an explicit on-device option for this routine. Settings can compare its headings and output against a small sample of existing processed archives without editing them.
