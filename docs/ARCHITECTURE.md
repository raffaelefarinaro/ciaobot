...
ciao/                          Python backend (Starlette).
  ...
  vault_index.py               Build/query memory-vault/INDEX.md and VOCABULARY.md from frontmatter and relative Markdown links in note bodies; owns the canonical `type:` set and the shared link helpers (`vault_link_ref`, `resolve_vault_link`, `markdown_destination`). CLI: `ciao vault-index`.
...
Delegates let a chat's agent spawn writable child chats (`delegate_spawn`) to work in parallel on a model it picks. A delegate is an ordinary chat carrying `spawned_from_chat_id`, so it keeps its own resumable session, full tool access, and permission cards in its own chat. When a delegate's turn ends, `ProjectChatManager._queue_delegate_wake` records the completion and `_flush_delegate_wake` delivers one wake turn to the supervisor after a 5s coalescing window, via `queue_message` (supervisor mid-turn) or `start_stream` (idle, or cold after a restart). Failures wake the supervisor too, but stops do not, or not always: `ChatStream.stopped_by` records who ended the turn, and only an agent-issued `chat_stop` reports back (as `STOPPED`, never `FAILED` — a stop arrives error-shaped, and `had_error` survives into teardown when no follow-up was queued). A human stop and the stop taken before archiving a delegate are both silent: the operator is taking the chat over, so a wake would start a turn on top of it. Each wake entry also carries harness-counted tool activity for the delegate's turn (`tool_event_count`, `write_tool_count`, `bash_tool_count`), bumped from the provider-neutral `ToolUseEvent` stream and surfaced on `delegates_list` rows alongside server-computed `age_seconds`/`idle_seconds` — `last_activity_at` is only stamped at turn boundaries, so it cannot separate a slow delegate from a dead one. Write counts under-report by construction (editing through bash is invisible to them); zero tool calls against a claim of success is the signal they exist for. A delegate's permission mode is capped at its supervisor's by `_child_mode`, which is a ceiling rather than a pin: a weaker request (a read-only `plan` delegate) is always honoured, and the per-workspace `delegate_max_mode` in `.runtime/workspaces.json` is the operator-only lever that raises it — deliberately unreachable from `workspace_update`, which is auto-approved, and only honoured when an agent *creates* a chat: `chat_update` passes `workspace_ceiling=False`, so no chat can raise its own mode with one auto-approved call. A stop only suppresses the wake for the turn it ended; the drive loop clears `stopped_by` before starting a queued follow-up turn. Delegate result-ready announcements (`chat_result_ready`, delayed Web Push / tray) are suppressed — the parent wake is the user-facing signal — but permission and AskUserQuestion pushes still fire for the child. Delegates cannot nest and are capped at 6 live per supervisor; the PWA nests them under their supervisor in the sidebar and emits `chat_delegates_reported` on `/ws/events`. Archiving a supervisor also archives its active delegate subchats, with the PWA stating that cascade in the archive menu/confirmation.
