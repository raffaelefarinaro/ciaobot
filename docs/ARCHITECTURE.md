...
The app repo above is the installable package. Installed state splits into two directories: the **workspace** (app data — config, `.env`, runtime state, chat metadata; default `~/.ciaobot`) and the **vault** (`CIAO_VAULT_ROOT`, the user's second brain of markdown notes, which can live anywhere on disk). The setup wizard asks only for the vault; the workspace is plumbing behind the Advanced disclosure. Legacy layouts keep the vault inside the workspace as `memory-vault/`.

Git follows the vault: setup initializes the vault as a git repository (`git init -b main`, a minimal `.gitignore` for `.DS_Store` and `.obsidian/workspace*`, and an initial commit) because the second brain is the user data worth versioning. Setup also ensures the workspace is a git repository: if the chosen folder is not already inside one, it runs `git init -b main`, writes a `.gitignore` guarding `.env`, `.runtime/`, `.claude/`, and `*.log`, and makes an initial commit. In both cases pre-existing repos are left untouched apart from appending any missing `.gitignore` guards, and a vault nested inside the workspace repo (legacy layout) is never double-initialized.
...
Every instance is identical (no primary/secondary, no cloud). Git follows the vault: sync and branch backup operate on the repo containing the vault root (`ciao/local_session.py::sync_root` — the vault's repo when the vault exists and is (in) a git repository, else the workspace root; legacy vault-inside-workspace layouts resolve to the same workspace repo either way). Ciaobot never creates or switches local branches: it works on whatever branch that checkout is currently on (`ciao/local_session.py::workspace_branch`). A background loop pushes that branch for backup; roots that are not git repositories or have no `origin` remote skip all git background work with a single INFO log (a fresh `ciao setup` initializes the vault and workspace as local repos, but they have no remote until the user adds one). Only the instance with `CIAO_DISPATCH_SCHEDULES` set (the always-on "main" device) dispatches scheduled automations, so schedules never double-fire when an occasional dev box is also running (`config.dispatch_schedules`).
...
**Package updates.** App code is a pip package, so workspace git handback never implies a deploy. `GET /api/package/status` reports `ciao.__version__` and a best-effort latest version from the GitHub releases API (`CIAO_GITHUB_REPO` overrides the repo), and `POST /api/package/update` installs the `.whl` asset of the latest GitHub release based on the active installation mode (pip venv) and restarts the server, allowing the Settings PWA panel to handle upgrades seamlessly. Updates never install from the PyPI index: the `ciao` name there belongs to an unrelated project.

**Setup readiness.** When `PWA_AUTH_TOKEN` is absent, Ciaobot starts in bootstrap mode using `~/.ciao/bootstrap` (or `CIAO_BOOTSTRAP_WORKSPACE`) and persists a generated temporary auth token under that workspace's `.runtime/`. `GET /api/setup-status` is public like startup status, because the first-run wizard needs it before a normal session exists. It reports workspace/vault/token/push-contact checks plus Claude Code, Ollama, and OpenRouter readiness probes, without returning secret values. The local first-launch URL `/?setup=<token>` redeems `.runtime/setup-token` on localhost only, sets the signed PWA session cookie, deletes the one-time token, and redirects to `/`. The wizard finishes with `POST /api/setup/finish`, which is bootstrap-only and localhost-only: `vault_root` is required (the wizard's one folder question) and `workspace` is optional, defaulting to `~/.ciaobot`. It writes the real workspace `.env` (recording `CIAO_VAULT_ROOT` as an absolute path when the vault lives outside the workspace), scaffolds the configured vault root, initializes vault and workspace git repos with their protective `.gitignore`s (see "Workspace layout"), creates the LaunchAgent plist and local `Ciaobot.app` shortcut, then requests the normal restart exit so launchd can relaunch into the real workspace. A foreground `ciao run` supervises that exit itself: when the server exits with the restart code, the CLI re-execs a fresh `ciao run` (picking up newly installed code) instead of dying.
