...
ciao/                          Python backend (Starlette).
  ...
  gws_skills.py                Regenerate the curated gws-* stock skills from `gws generate-skills` (strips the generator's "Community & Feedback Etiquette" bloat from gws-shared; leaves the curated set on disk as the source of truth). Used by `ciao/release.py`.
