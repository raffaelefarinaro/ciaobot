...
6. **The desktop gate — run it if anything under `desktop/` changed.** `pytest tests/` and the `npm run build`s do not compile Rust, build the Swift native sidecar, or assemble `Ciaobot.app`, so a desktop change can be green locally and still fail CI's `build-desktop` job — after the tag exists. `./scripts/check-desktop.sh` runs the same commands CI does (sidecar build, `cargo fmt --check`, `cargo clippy -D warnings`, `cargo test`, then an aarch64 `tauri build`) and asserts the sidecar ends up bundled, signed, and runnable inside the built app. `prepare-release` runs it as part of its checks, but run it yourself first: finding a clippy failure before the release branch exists is much cheaper. Needs Rust 1.90.0 and `swiftc`; `--fast` skips the bundle build.
