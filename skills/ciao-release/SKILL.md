...
3. That fires `publish.yml`, which ships **both the engine and the desktop app from the same tag**:
   ...
   - `build-desktop` (macos) — `npm run tauri build -- --target universal-apple-darwin`, then attaches `Ciaobot_<version>_universal.dmg`, `…app.tar.gz`, `…app.tar.gz.sig`, and a generated `latest.json` (the updater manifest the app polls). Needs the `TAURI_SIGNING_PRIVATE_KEY` / `…_PASSWORD` secrets (these are the *updater* signing keys, unrelated to Apple code signing). Developer ID signing + notarization are opt-in on `APPLE_CERTIFICATE`, `APPLE_CERTIFICATE_PASSWORD`, `APPLE_SIGNING_IDENTITY`, `APPLE_ID`, `APPLE_PASSWORD`, `APPLE_TEAM_ID` all being set; absent those the app is ad-hoc signed and **not** notarized, and the job emits a warning. Every run logs `notarized: yes|no` — check that rather than inspecting the DMG. Notarization must happen at build time: notarytool rejects an ad-hoc bundle, so it cannot be added to an already-built artifact.
...
- **An un-notarized cask install needs manual approval on first launch.** `brew install --cask` sets `com.apple.quarantine` (seen as `0181;…;Homebrew\x20Cask;…`), and macOS will not launch a quarantined, non-notarized app without a user approving it in System Settings → Privacy & Security. There is no unattended path: CI clears the attribute with `xattr -dr com.apple.quarantine`, which users cannot do for themselves as part of a normal install. Enabling the Developer ID secrets on `build-desktop` removes the prompt; until then, expect first-launch friction and do not mistake it for a broken build.
- **`pgrep` proves the process exists, not that the app runs.** The v0.6.3/v0.6.4 `release-smoke` failures looked like a broken engine cold start; a stack sample proved the app was pinned in `_dyld_start` with a 96K footprint 35s after launch, having never reached `main()`. Cause: the app is ad-hoc signed and **not notarized**, and `brew install --cask` sets `com.apple.quarantine`, so the first launch waits on a Gatekeeper assessment nothing is there to approve. CI installs the cask with `--no-quarantine` for this reason. When the probe fails, read the stack sample first — `launchctl`/engine theories are worthless if no Rust code ever ran.
