...
6. **The desktop gate — run it if anything under `desktop/` changed.** `pytest tests/` and the `npm run build`s do not compile Rust or assemble `Ciaobot.app`, so a desktop change can be green locally and still fail CI's `build-desktop` job — after the tag exists. `./scripts/check-desktop.sh` runs the same commands CI does (`cargo fmt --check`, `cargo clippy -D warnings`, `cargo test`, then an aarch64 `tauri build`) and asserts the bundle ends up signed and runnable. `prepare-release` runs it as part of its checks, but run it yourself first: finding a clippy failure before the release branch exists is much cheaper. Needs Rust 1.90.0; `--fast` skips the bundle build.
