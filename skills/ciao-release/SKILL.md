...
- **Notarization is deliberately not done, so first launch needs a user gesture.** There is no Apple Developer certificate for this project and none is planned. Users installing the cask must Control-click Ciaobot.app → Open once; the cask `caveats` in `scripts/update-homebrew-tap.sh` say exactly that. Don't "fix" a first-launch report by adding a quarantine-stripping `postflight` to the cask without deciding that tradeoff explicitly — it silently removes the Gatekeeper check the same caveats tell users not to disable.
