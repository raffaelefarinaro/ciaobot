...
- **Node floor: activate 22 in the shell that runs the release tool.** `_run_checks` shells out to `npm run test` in `web/`, which runs `scripts/check-node.mjs` and hard-fails below `^20.19 || ^22.13 || >=24` — so with a default Node of 20.x the release dies at `ReleaseError: command failed (1): npm run test`, *after* it has already bumped the version files and regenerated the changelog. `nvm use 22` is per-shell and is not inherited by a release run started from a different one: activate it in the same command (`. "$NVM_DIR/nvm.sh" && nvm use 22 && … -m ciao.release …`), or set `nvm alias default 22`. Then clean up per the double-bump trap below before retrying.
