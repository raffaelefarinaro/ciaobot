...
- `CIAO_GITHUB_TOKEN` (also honors `GITHUB_TOKEN` / `GH_TOKEN`): personal access token used to authenticate the update-check calls to the GitHub API. Optional; when set it raises GitHub's rate limit from 60 to 5000 requests/hour, which avoids intermittent "rate limit reached" update-check failures on shared or NAT egress IPs. No scopes are required (public read only).
