...
**Required for a configured workspace:** `PWA_AUTH_TOKEN` — the dashboard password. Password protection is on by default; see `PWA_AUTH_REQUIRED` below for the opt-out. `CIAO_PUSH_CONTACT` is optional: leave it empty and Web Push still works, using the placeholder subject `mailto:ciaobot@localhost`.
...
- `CIAO_PUSH_CONTACT` (optional): push notification contact string for the Web Push VAPID subject, for example `mailto:you@example.com`. Empty (or unset) falls back to the placeholder `mailto:ciaobot@localhost`, so Web Push works out of the box; the push service never contacts the address. It is set only in `.env` (or with `ciao setup --push-contact` when setup first writes `.env`); there is no Settings field for it. The macOS menu-bar companion posts local alerts from the runtime notification log either way. Read mutations emit a clear control for matching delivered PWA and macOS notifications.
...
- `CIAO_AUTO_SYNC_ON_START=true` enables the automatic `git pull --rebase` on server startup (disabled by default). Only an explicit affirmative value (`true`/`1`/`yes`/`on`) opts in; unset, blank, or unrecognized values stay disabled.
...
- `CIAO_PUSH_CONTACT`: push notification contact string, used as the VAPID subject. Optional; empty falls back to `mailto:ciaobot@localhost` and Web Push still delivers.
...
- `CIAO_PROVIDER_IDLE_TIMEOUT`: seconds a chat may sit idle before its provider is disconnected and reclaimed (default `900`). This matters most for opencode, which runs one `opencode serve` process per chat; without the sweep those processes live until the server restarts. A chat is never reclaimed while it has a live stream, a between-turns drain, a retry loop, a pending background wake, or a parked question/approval card. Reclaiming releases only the provider — the chat's `session_id` is persisted, so the next turn reconnects and resumes. A zero or unparseable value falls back to the default.
- `CIAO_PROVIDER_REAP_INTERVAL`: seconds between idle-provider sweeps (default `120`). Same fallback behaviour as above.
