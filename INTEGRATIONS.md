...
- `CIAO_AUTO_SYNC_ON_START=true` enables the automatic `git pull --rebase` on server startup (disabled by default). The dataclass default and the `.env` fallback both treat an unset value as disabled.
