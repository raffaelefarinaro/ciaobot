...
The endpoint is mounted at `http://127.0.0.1:<PWA_PORT>/agent/v1/{op}`. Do not place a static token in `.mcp.json`: Ciaobot generates a scoped short-lived token and configures its managed provider process. See [docs/AGENT_CLI.md](docs/AGENT_CLI.md).
