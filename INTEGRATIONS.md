...
**Credentials persistence in `.env`.** GWS configs (`secrets/gws/` and `secrets/gws-personal/`) are gitignored and can be lost during git cleans or device migrations. They can be backed up as base64-encoded tarballs directly in `.env` under `GWS_PERSONAL_SECRETS_B64` and `GWS_WORK_SECRETS_B64`.
- **Automatic Backup:** Spawning authentication via `scripts/gws-auth-helper.py` automatically updates these `.env` variables upon successful authorization. You can also back them up manually at any time by running `python3 scripts/gws-secrets.py backup <personal|work>`.
- **Automatic Restore:** If GWS credentials are missing when running a command, `scripts/gws-profile.sh` automatically restores them from `.env` by invoking `python3 scripts/gws-secrets.py restore <personal|work>`.
...
`ciao setup` writes the initial `.env` into the selected workspace, seeds stock agents, commands, schedules, and the default vault, renders `~/Library/LaunchAgents/com.ciao.server.plist`, and creates `~/Applications/Ciao.app`. The app shortcut opens `http://localhost:<port>/?setup=<token>`; the server redeems `.runtime/setup-token` once on localhost, sets the signed session cookie, then deletes the token. By default setup prints the launchd load command without starting the service; use `--load-launchd` to run `launchctl`. `ciao auth <claude|pi|ollama>` runs the provider login command in Terminal; `--print-only` shows the command for the setup wizard. `GET /api/setup-status` reports required local config plus Claude, Pi, and Ollama readiness so the wizard can poll after terminal OAuth commands. In bootstrap mode, `POST /api/setup/finish` accepts the wizard's final local choices, writes the real workspace `.env`, scaffolds the configured `CIAO_VAULT_ROOT`, refreshes the LaunchAgent and `Ciao.app` shortcut, and requests the restart exit for supervisor relaunch.
...
**OpenRouter:** `OPENROUTER_API_KEY` (optional). When set, the Settings → Models tab uses the OpenRouter critique/review panel (`openrouter/anthropic/claude-3.7-sonnet`, `openrouter/google/gemini-2.5-pro`, `openrouter/minimax/minimax-m3`, `openrouter/zai/glm-5.2`) instead of the default panel. Used by `adversarial-review` and the `critique_models` default.
...
## Ciao Server Operation

Runtime config for the Ciao server itself (PWA, schedules, deploy).
...
- Ciao sends baseline security headers from the Starlette app, including CSP, `X-Content-Type-Options`, `Referrer-Policy`, and frame denial.
...
- `./gws-auth-bridge.sh`: helper for headless `gws` auth when the keyring backend fails.
- `scripts/gws-auth-helper.py`: interactive headless OAuth re-authentication when `gws auth login` cannot open a browser.
- `CIAO_OLLAMA_MODELS`: comma-separated list of Ollama-routed model IDs (e.g. `kimi-k2.7-code:cloud,deepseek-v4-pro:cloud,deepseek-v4-flash:cloud`). When the picker selects one of these, Ciao injects `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_API_KEY=""`, `ANTHROPIC_BASE_URL` into the CLI subprocess so the same Claude Code stack hits Ollama. The explicit empty `ANTHROPIC_API_KEY` is required (it forces the bundled CLI to pick `ANTHROPIC_AUTH_TOKEN` as the auth source), so do not set `ANTHROPIC_API_KEY` in `.env`. An empty value in the parent env still trips the CLI's "another auth source is set" warning and breaks downstream subagents like depcheck. Listed models are also auto-appended to the model picker (no need to also add them to `CLAUDE_MODELS`).
...
- `CIAO_OLLAMA_LOCAL_URL`: base URL of the *local* Ollama daemon used for locally-installed models, independent of `CIAO_OLLAMA_URL` (which may point at ollama.com when a cloud key is set). Default `http://localhost:11434`. Local models route here with the literal `ollama` token; cloud models keep going to `CIAO_OLLAMA_URL`.
- `CIAO_OLLAMA_LOCAL_MODELS`: comma-separated list of local-daemon model IDs to pin into the pickers (e.g. `gemma4:12b-it-qat`). Usually unnecessary: at startup Ciao auto-discovers installed models via `GET <local_url>/api/tags` and merges them in (models already in `CIAO_OLLAMA_MODELS` keep their cloud routing). Local models appear in the personal Claude bucket and the Pi bucket, badged as local.
...
- `CIAO_OLLAMA_TITLE_MODEL`: cheap model used to auto-title Ollama-routed chats (the chat's own `:cloud` model is usually subscription-gated and overkill for 50-token titles). Default `ministral-3:3b` (works on Free tier; smallest model in the catalog). Other free-tier-friendly options: `gemma3:4b`, `qwen3:8b`, `gpt-oss:20b`, `nemotron-3-nano:30b`. Anthropic chats keep titling via `CIAO_TITLE_MODEL` (default `haiku`) and ignore this knob. When the `apfel` CLI is on PATH, the title call uses it first. If `apfel` is unavailable or fails, and the `pi` binary is on PATH, the title call goes through `run_pi_oneshot()` (Pi RPC subprocess) instead of the Claude Agent SDK; Claude SDK with Ollama env-injection remains the fallback.
...
- `CIAO_INSIGHTS_MODEL`: model ID for insights extraction, routed through the Ollama Anthropic-compatible API. Default `deepseek-v4-flash:cloud`. This model is fixed at the server level and bypasses the Ollama models allowlist. Uses existing `CIAO_OLLAMA_*` config (base URL, API key). When the `pi` binary is on PATH, the call goes through `run_pi_oneshot()` against the same model ID; Claude SDK with Ollama env-injection is the fallback when Pi is unavailable.
...
- `CIAO_SKILL_EVOLUTION_MODEL`: model ID used by the weekly skill-evolution pass when proposing skill edits from trajectory data. Default `kimi-k2.7-code:cloud` (sonnet/opus-tier, runs via Ollama). A small model can't reason over a 15 KB SKILL.md plus trajectory JSON. Keep this at a capable model.
- `CIAO_REVIEW_MODELS`: comma-separated list of model IDs for the critique / adversarial-review skill. Overrides the script's default panel. Runtime-overridable from the PWA (Settings → Models, persisted in `.runtime/app_settings.json` under `critique_models`).
...
- `CIAO_OLLAMA_HAIKU_MODEL` / `CIAO_OLLAMA_SONNET_MODEL` / `CIAO_OLLAMA_OPUS_MODEL`: per-tier Ollama model overrides for Claude chats whose effective `model_bucket` is `personal` and that select the aliases `haiku`, `sonnet`, or `opus`. The alias resolves at runtime only when the target model is in `CIAO_OLLAMA_MODELS`; `work` bucket chats keep Anthropic aliases. Defaults: `deepseek-v4-flash:cloud` (haiku), `kimi-k2.7-code:cloud` (sonnet), `minimax-m3:cloud` (opus). `glm-5.2:cloud` (Z.ai 753B, MIT, 1M context) is a strong alternative for the `opus` tier if you want open-weight agentic coding at lower cost than Anthropic.
- `CIAO_PI_MODELS`: comma-separated list of Pi-native add-on model IDs (extras the Pi bucket should expose on top of `CIAO_OLLAMA_MODELS`). Models listed here surface only under the Pi provider in the picker; if the same ID is also in `CIAO_OLLAMA_MODELS`, it surfaces under both Claude and Pi (selecting the provider chooses the spawn path). Leave empty to expose only the Ollama set under Pi. IDs prefixed with one of Pi's built-in providers (`openai/`, `google/`, `azure/`, `openai-codex/`, `anthropic/`) are dispatched as `--provider <prefix> --model <rest>` (e.g. `openai-codex/gpt-5.5` → `--provider openai-codex --model gpt-5.5`); bare IDs fall through to `CIAO_PI_PROVIDER` (default `ollama`).
- `CIAO_PI_PROVIDER`: backend that Pi talks to. Default `ollama` (Pi RPC against the local Ollama daemon or Ollama Cloud). Change only if you point Pi at a different backend.
- `CIAO_PI_OLLAMA_URL`: base URL Pi uses to reach Ollama. Default `http://localhost:11434`. Set to `https://ollama.com` (or a self-hosted relay) when Pi should hit cloud directly.
- `CIAO_PI_DEFAULT_MODEL`: default model for new chats created under the Pi provider. Empty falls back to the first entry of the Pi bucket (`CIAO_PI_MODELS` then `CIAO_OLLAMA_MODELS`).
- `CIAO_PI_SESSION_DIR`: override for the root of Pi's session directory. Default `~/.pi/agent/sessions/`; each chat gets a `<chat_id>` subdirectory that is reaped when the chat is archived or deleted. Only set this in tests or when isolating sessions.
...
- `CIAO_TRANSCRIPTION_ENGINE`: voice dictation engine, `cloud` (default; OpenAI API, needs `OPENAI_API_KEY`) or `local` (mlx-whisper on-device, free, Apple Silicon only; bundled in the default bootstrap, this repo now targets macOS only). A failing or missing local engine falls back to cloud when a key is configured. Runtime-overridable from Settings → Models.
...
**Note:** `./gws-auth-bridge.sh` is a helper for headless `gws` auth when the keyring backend fails.

### Injected CLI context variables

The Ciao server injects the following environment variables into every spawned agent CLI subprocess (e.g. `claude` or `pi`):
...
- `CIAO_PROVIDER`: the provider name (either `claude` or `pi`).
...
See `docs/runbooks/add-ollama-model.md` for the full recipe (env-var wiring, alias and one-shot model overrides, Pi-bucket exposure, restart, verification).
...
Ciao runs on macOS under launchd.

- `ciao setup --workspace <path> --load-launchd` renders and loads the LaunchAgent.
- `deploy/com.ciao.server.plist` is a generic template mirror; the packaged source of truth is `ciao/stock/deploy/com.ciao.server.plist.tmpl`.
...
- See `deploy/README.md` for the macOS launchd template notes.
