...
defaults, model routing, title generation, insights, schedules, and the critique
...
- `CIAO_WORKSPACES`: JSON workspace registry. Preferred shape is a list of objects with `name`, `vault_root`, `default_provider`, `default_model`, `disallowed_tools`, `claude_ai_mcps`, and `gws_profile`. `vault_root` is relative to `CIAO_WORKSPACE` unless absolute. It is an internal/setup migration field: fresh setup and ordinary PWA workspace creation derive `<CIAO_VAULT_ROOT>/<name>`, while existing-folder setup preserves the selected root until a model-guided migration updates the registry. Later Settings updates preserve the stored path. If unset, Ciaobot reads `.runtime/workspaces.json`; if that is also missing, it falls back to the legacy `personal` and `work` workspace definitions. Schedules assigned to a workspace inherit its current `default_provider` and `default_model` on every run unless an explicit override is stored. Example: `[{"name":"default","vault_root":"memory-vault/default","default_provider":"claude","default_model":"opus","gws_profile":"personal"}]`.
...
- `CIAO_DISALLOWED_TOOLS_PERSONAL` / `CIAO_DISALLOWED_TOOLS_WORK`: CSV of extra (non-connector) tools to deny in the legacy `personal`/`work` workspace, on top of the connector set controlled by the toggle. Literal `none` clears the extras. Every workspace defaults to denying the PWA-irrelevant harness tools (`EnterPlanMode`, `ExitPlanMode`, `DesignSync`, `NotebookEdit`, `CronCreate`, `CronDelete`, `CronList`, `ScheduleWakeup`, `PushNotification`, `RemoteTrigger`, `Skill(schedule)`, `Skill(loop)`) since the PWA is the plan-approval, notification, and scheduling surface and the CLI equivalents just duplicate it or have no UI; the two bundled automation skills are denied for the same reason, so "loop this every hour" routes to the `loop_*` MCP tools instead of a cloud routine Ciaobot cannot see. Denying a skill blocks execution but leaves it in the model's listing, so the engine also passes an inline `--settings` layer with `{"skillOverrides": {"schedule": "off", "loop": "off"}}` (see `HARNESS_DISABLED_SKILLS` in `ciao/execution_modes.py`), which drops both out of the model's context and the slash-command picker. That layer is additive: user, project, and local settings — and therefore every workspace skill, agent, and command — still load. Set the CSV to `none` to opt a workspace out of the harness set too.
...
- `CIAO_APPLE_INTELLIGENCE`: set to `true`/`1` to opt in to the **beta** Apple Intelligence on-device model (the "Local (free)" option for chat titles, Session insights, and re-entry summaries). Default is off: the feature is beta and disabled by default. It is also switchable at runtime from Settings → Models (persisted in `.runtime/app_settings.json`); an explicit Settings toggle overrides this env default in either direction. When off, the `apple` sentinel reports unavailable and routines fall back to their cloud model.
...
- `CIAO_MEMORY_CHAR_LIMIT`: advisory cap (chars) on the `ciao:memory` region in the workspace `CLAUDE.md`. Default `2200`. Nothing refuses a write at edit time; `os_audit` and nightly memory curation report/consolidate when over cap.
...
- `CIAO_DISALLOWED_TOOLS_PERSONAL` / `CIAO_DISALLOWED_TOOLS_WORK`: legacy per-workspace tool denylist knobs. They seed the default `personal`/`work` entries when `CIAO_WORKSPACES` and `.runtime/workspaces.json` are absent. **Both workspaces default to denying the PWA-irrelevant harness tools** (`EnterPlanMode`, `ExitPlanMode`, `DesignSync`, `NotebookEdit`, `CronCreate`, `CronDelete`, `CronList`, `ScheduleWakeup`, `PushNotification`, `RemoteTrigger`, `Skill(schedule)`, `Skill(loop)`); **personal additionally denies the 8 claude.ai connector MCPs** (Airtable, Asana, Atlassian, Google_Cloud_BigQuery, Salesforce, Sentry, Slack, incident_io). Other MCP servers (e.g. a self-hosted `n8n_mcp`) are not denied by default — Ciaobot ships no opinion on them, so each workspace's "Extra disallowed tools" field decides. Three-state knob: unset / blank → use defaults; CSV → custom denylist (replaces the harness defaults); literal `none` → opt out (zero extras on the workspace, used to restore claude.ai MCPs on personal). Tool names follow the SDK convention: `mcp__servername` blocks the whole server, `mcp__servername__toolname` blocks one tool, `Bash` blocks the Bash builtin, `Skill(name)` blocks one skill, etc.
...
- **Auto tier-fallback on capability errors**: when a turn's primary model returns a 4xx that means the model itself cannot handle the input (`does not support image input`, `does not support tool`, `context length exceeded`, etc.), the chat drive loop silently retries the request on the nearest neighbor tier (cheaper first, then more capable). The ladder is `fable → opus → sonnet → haiku`, and it applies only when the chat named a bare tier alias — a chat pinned to a concrete model id opted into that model, so it is never silently swapped. The full original error text is logged at WARNING before the retry, and the PWA shows a `status` event between attempts. Rate limits, auth errors, content-filter rejections, and 5xx errors do NOT trigger the retry — those need operator attention. Codex is excluded. One retry per turn; the new model starts a fresh session id (the failed session is not resumed). Implemented in `ciao/model_tiers.py::next_tier_for_failure` and `ciao/web/project_chats.py::ProjectChatManager.stream_chat`.
