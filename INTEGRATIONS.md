...
- `CIAO_MEMORY_ENABLED`: set to `false`/`0`/`no`/`off` to skip injecting the `ciao:memory` / `ciao:profile` regions from the workspace `CLAUDE.md` into Claude and Codex system prompts. Default enabled. Useful when debugging prompt-cache behavior or for one-off chats that should run with no injected persisted-memory snapshot.
